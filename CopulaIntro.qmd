...
ll_function <- function(parameters, data_y) {
  ...
  dim(u) <- c(nrow(data_y), ncol(data_y))


  # Map uniform variates to normal scores
  q <- qnorm(u)
  dim(q) <- c(nrow(data_y), ncol(data_y))
  Sigma <- matrix(c(1, rho, rho, 1), nrow = 2)
  copula_log_lik <- sum(RTMB::dmvnorm(q, Sigma = Sigma, log = TRUE)) - sum(dnorm(q, log = TRUE))
...
```{r include=FALSE}
ll_function <- eval(parse(text = deparse(ll_function)))
```
